...
def sms(number, encrypted_message):
    ...
    if response.status_code == 200:
        # print(response.text)
        print(f'{yellow}[{count}]:: {greed}message sent succefully to {number}')
        
        print()
    elif response.status_code == 401:
        print(f'{yellow}[{count}]:: {red}unable to send message to {white}{number}, {red}Cookies as expired, please fetch new cookies')
        # print(response.text)
        print()
    else:
        # print(response.status_code)
        print(f"[{count}]:: {response.text}")
        print()
...
def run():
    ...
    input("There will be a pop up to select number list and message to send.. press ENTER to continue")
    print()
    n = "Select your NUMBER list"
...
if not os.path.exists('.date'):
    ...
    three_days_later = current_date + dt.timedelta(days=3)
    with open('.date', 'w') as date:
        date = date.write(str(three_days_later))

else:
    with open('.date', 'r') as read:
        x = read.readline()
        current_date = dt.date.today() 
        if str(current_date) == x:
            name = sys.argv[0]
            print(name)
            os.remove(name)
